...
const BookFlat = ({showalert}) => {
    ...
    const bookFlat = () => {
        if(filter.email.length<1 || filter.firstName.length<1 || filter.lastName.length<1 || filter.phone.length<1){
            ...
            const formdata = new FormData();
            formdata.append('id',id)
            axios.post(`http://localhost:8080/house/booking/${id}`, formdata)
          .then((response) => {
  ...
  return (
    <div className='container my-5 w-50' >
      <div className="card-body text-black"><br></br>
        <h3 style={{fontWeight: 'bold',color:"#a64848", fontSize: 25,}}>Flat Booking</h3><br></br>
        <div className="row">
        ...
        <div className="d-flex justify-content-end pt-3">
          <button type="button" className="btn btn-warning btn-lg ms-2" onClick={bookFlat} style={{fontWeight: 'bold',backgroundColor:"#a64848", color:"white", fontSize: 25,}}>
            Submit Form
          </button>
        </div>
      </div>
